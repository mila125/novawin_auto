...
lados_rectangulos = np.append(lados_rectangulos, [105,734])
lados_rectangulos = np.append(lados_rectangulos, [180, 307])
lados_rectangulos = np.append(lados_rectangulos, [50, 495])
lados_rectangulos = np.append(lados_rectangulos, [10, 93])
lados_rectangulos = np.append(lados_rectangulos, [0, 0]) #finaliza la sequencia 
def draw_same_squares_rectangle(perimetro,cantidad,lados_rectangulos,numeros_derecha,lado_cuadrado,coordinates,t,numeros): 
   ...
   #draw_square( h,i,j,numeros_derecha[2]-lados_rectangulos[0],numeros_derecha[3]-lados_rectangulos[h+1],numeros_derecha[2]-lados_rectangulos[0],numeros_derecha[3],lado_cuadrado,numeros_derecha,lados_rectangulos,espacio_del_cuadrado,numeros_derecha_cuadrados,numeros_cuadrado,t,direction_flag) 
    ...
    while i<lados_rectangulos[h+1]: #y
     ...
     if (i % espacio_del_cuadrado == 0) and (i >= espacio_del_cuadrado):
            ...
            numeros = np.append(numeros, coordinates_i)  #para no dejar espacios vacios
              
                   
            while j<lados_rectangulos[h]: #x 
             print(f"j: {j}") 
             numeros_derecha = draw_square(t, coordinates, lado_cuadrado, numeros_derecha_cuadrados)
...
def work_with_squares(t,coordinates,lado_cuadrado,numeros, numeros_derecha,rectangle_width, rectangle_height,lados_cuadrado,numeros_cuadrado,numeros_cuadrado_derecha,distancia_hasta_el_borde_x,distancia_hasta_el_borde_y,old_width,old_heigth,h,lados_rectangulos,perimetro,distancia_hasta_el_borde_old,nivel):
    ...
    if(rectangle_height > distancia_hasta_el_borde_y or rectangle_width > distancia_hasta_el_borde_x ) :
...
def draw_nested_rectangles():
    ...
    while(h<(lados_rectangulos.size-2)):
     ...
     distancia_hasta_el_borde_y = lados_rectangulos [1]-numeros_derecha[h+1]
     distancia_hasta_el_borde_x = lados_rectangulos [0]-numeros_derecha[h]
    ...
    work_with_squares(t,coordinates,min_width,numeros,numeros_derecha,lados_rectangulos[h],lados_rectangulos[h+1],lados_cuadrado,numeros_cuadrado,numeros_cuadrado_derecha,distancia_hasta_el_borde_x,distancia_hasta_el_borde_y,main_width,main_height,h-2,lados_rectangulos,perimetro,distancia_hasta_el_borde_old,nivel)     
